...
export const resendOTPApi = (email: string) => {
  return Api.post<void, VerifyOTPResponse>(
    `/user/resend-otp?email=${email}`,
  );
};

export const updateUserApi = async (body: UserData) => {
  const session = await getUser();
  return Api.patch<UserData, AuthResponse>(`/user/${session?._id}/profile`, body, true);
};
...
export const passwordResetApi = (body: ResetPassword) => {
  return Api.post<ResetPassword, AuthResponse>(
    `/user/password/reset`,
    body,
  );
};
