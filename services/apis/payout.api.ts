...
import { PayoutRsp, TransactionRsp } from "@/types/payout";
import { queryBuilder } from "@/utils/helper";

export const getAllTransactionsApi = async (page = "1", limit = "10") => {
  const user = await getUser();
  return Api.get<TransactionRsp>(
    `/transactions/user/${user?._id}?${queryBuilder({ page, limit })}`,
  );
};
